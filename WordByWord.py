    ...
    def _get_dim_names(self):
        ...
        for word in self.lexicon:
            for feat in self.features:
                per_position.append('_'.join([word, feat]))
        ...
        links = []
        for pos_nr, word in product(range(self.max_sent_length), self.lexicon):
            other_positions = [x for x in range(self.max_sent_length)
                               if x != pos_nr]
            # Any word can appear at any position, so use whole lexicon here
            for op, ow in product(other_positions, self.lexicon):
